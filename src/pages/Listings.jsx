...
function ListingSkeleton() {
  return (
    <div className="listing-skeleton-list">
      {[1, 2, 3, 4].map(i => (
        <div className="listing-skeleton-card" key={i}>
          <div className="skeleton-img skeleton-pulse" />
          <div className="skeleton-body">
            <div className="skeleton-line skeleton-pulse" style={{ width: '40%', height: '14px', marginBottom: '10px' }} />
            <div className="skeleton-line skeleton-pulse" style={{ width: '80%', height: '20px', marginBottom: '8px' }} />
            <div className="skeleton-line skeleton-pulse" style={{ width: '55%', height: '13px', marginBottom: '20px' }} />
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <div className="skeleton-line skeleton-pulse" style={{ width: '100px', height: '34px' }} />
              <div className="skeleton-line skeleton-pulse" style={{ width: '90px', height: '34px' }} />
            </div>
          </div>
        </div>
      ))}
...
export default function Listings() {
  ...
  // Memoized filter + sort for performance
  const sortedFiltered = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (sortBy === 'price-asc') {
        return (Number(a.price.toString().replace(/[^0-9]/g, '')) || 0) - (Number(b.price.toString().replace(/[^0-9]/g, '')) || 0)
      }
      if (sortBy === 'price-desc') {
        return (Number(b.price.toString().replace(/[^0-9]/g, '')) || 0) - (Number(a.price.toString().replace(/[^0-9]/g, '')) || 0)
      }
      return 0
    })
  }, [allListings, search, typeFilter, statusFilter, locationFilter, budgetMin, budgetMax, bedsFilter, sortBy])
          ...
          <div className="listings-layout">
            ...
            <div className="listings-main">
              <div className="listings-sort-bar">
                <p className="listings-count">
                  {loading ? (
                    <span style={{ color: 'var(--text-muted)' }}>Loading properties…</span>
                  ) : (
                    <>Showing <strong>{sortedFiltered.length}</strong> properties</>
                  )}
                </p>
            ...
            <aside className="listings-sidebar">
              <div className="sidebar-section">
                ...
                <button className="sidebar-filter-btn" id="sidebar-apply-btn" onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }) }}>Apply Filters</button>
              </div>
